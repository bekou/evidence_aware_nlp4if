...
class FCModel(nn.Module):
    ...
    def forward(self, x):
        ...
        #print ("here2self.activation(layer(x))")
        #print (x)
        #print (x.shape)
        return x
...
class LTRModel(nn.Module):
    ...
    def forward(self, x, mask, indices):
        ...
        """
        #print ('self.output_layer(self.prepare_for_output(x, mask, indices))')
        #print (self.output_layer(self.prepare_for_output(x, mask, indices)))
        #print (self.output_layer(self.prepare_for_output(x, mask, indices)).shape)
        #print ("here3")
        #print (x)
        #print ("here3xshape")
        #print (x.shape)
        #print ("here3outputshape")
        #print (self.output_layer(self.prepare_for_output(x, mask, indices)).shape)
...
class OutputLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        #print ('self.activation(self.w_1(x).squeeze(dim=2))')
        #print (self.activation(self.w_1(x).squeeze(dim=2)))
        #print (self.activation(self.w_1(x).squeeze(dim=2)).shape)
...
def make_model(fc_model, transformer, post_model, n_features):
    ...
    if transformer:
        #print ("transformertransformertransformertransformertransformer")
