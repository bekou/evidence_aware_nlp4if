...
def eval_model(model, validset_reader):
    model.eval()
    all_predict = dict()
    for inp_tensor, msk_tensor, seg_tensor, ids, evi_list,inp_anc_tensor, msk_anc_tensor, seg_anc_tensor in validset_reader:
        probs_anc = model(inp_anc_tensor, msk_anc_tensor, seg_anc_tensor)
        probs = model(inp_tensor, msk_tensor, seg_tensor)
        
        pdist = nn.PairwiseDistance(p=2)
       
        dist = pdist(probs_anc, probs)
        probs = probs.tolist()
        probs_anc=probs_anc.tolist()
        dist=dist.tolist()
        assert len(probs) == len(evi_list)
        assert len(probs) == len(probs_anc)
        
        for i in range(len(dist)):
            if ids[i] not in all_predict:
                all_predict[ids[i]] = []
            #if probs[i][1] >= probs[i][0]:
            all_predict[ids[i]].append(evi_list[i] + [dist[i]])
            #print (evi_list[i] + [dist[i]])
    return all_predict
