...
def eval_model(model, validset_reader):
    ...
    for inp_tensor, msk_tensor, seg_tensor, lab_tensor in validset_reader:
        ...
        preds.extend(np.argmax(prob, axis=1).flatten().tolist())
        labels.extend(lab_tensor.flatten().tolist())
        
        
    dev_accuracy = correct_pred / validset_reader.total_num
    logger.info('\n{0}'.format(classification_report(labels, preds)))
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for inp_tensor, msk_tensor, seg_tensor, label_tensor in trainset_reader:
            ...
            if global_step % (args.eval_step * args.gradient_accumulation_steps) == 0:
                ...
                eval_acc,eval_f1 = eval_model(model, validset_reader)
                logger.info('Dev f1: {0}'.format(eval_f1))
                if eval_f1 >= best_f1:
                    best_f1 = eval_f1
                ...
                if eval_acc >= best_acc:
                    ...
                    #torch.save({'epoch': epoch,
                    #            'model': model.state_dict()}, save_path + ".best.pt")
                    logger.info("Best epoch {0}, best acc {1}".format(epoch, best_acc))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid_batch_size", default=16, type=int, help="Total batch size for predictions.")
    ...
    parser.add_argument("--slate_length", type=int, default=20, help='Evidence num.')
